...
namespace Plattar {
	public class Exporter : EditorWindow {
		...
		[MenuItem("Plattar/Export Active Scene")]
		static void ExportScene()
        {
			GameObject[] rootObjects = SceneManager.GetActiveScene().GetRootGameObjects();
			if (rootObjects.Length == 0)
            {
				EditorUtility.DisplayDialog("Failed Export", "Active Scene is Empty", "OK");
			}
			if (GenerateGLTFZipped(rootObjects, SceneManager.GetActiveScene().name) != null)
			{
		...
		void OnGUI() {
			...
			if (selectedObject == null) {
				...
				if (GUILayout.Button($"Export {selectionName} to GLTF")) {
					if (GenerateGLTFZipped(new GameObject[] { selectedObject }) != null) {
						EditorUtility.DisplayDialog("Successful Export", "GLTF Exported and Zipped Successfully", "OK");
					}
				}
		 ...
		 */
		public static Tuple<string, string, string> GenerateGLTFZipped(GameObject[] selectedObjects, string nameOverride = "", string outputPath = "") {
			var path = GenerateGLTF(selectedObjects, nameOverride, outputPath);
			...
			if (!path.Item1.Contains(path.Item2))
			{
		 ...
		 */
		public static Tuple<string, string, string> GenerateGLTF(GameObject[] selectedObjects, string nameOverride, string fullpath, bool binary = false) {
			string selectionName = string.IsNullOrEmpty(nameOverride) ? selectedObjects[0].name : nameOverride;

			if (string.IsNullOrEmpty(fullpath))
			{
				fullpath = EditorUtility.SaveFilePanel("glTF Export Path", PlattarExporterOptions.LastEditorPath, selectionName, "zip");
			}
			...
			if (!string.IsNullOrEmpty(path)) {
				var newpath = path /*+ "_export_gltf"*/;
				DirectoryInfo info = Directory.CreateDirectory(newpath);

				if (info.Exists) {
					if (PlattarExporterOptions.ExportAnimations == true) {
						var exporter = new GLTFEditorExporter(selectedObjects.Select(GO => GO.transform).ToArray());
						exporter.enableAnimationClips(PlattarExporterOptions.ExportAnimationClips);
                        if (binary)
						{
							exporter.SaveGLB(newpath, selectionName);
