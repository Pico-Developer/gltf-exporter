...
namespace UnityGLTF.Extensions
{
	...
	{
        public static Matrix4x4 MatrixBetween(Transform viewTransform, Transform modelTransform)
        {
            var viewMatrix = viewTransform != null ? viewTransform.worldToLocalMatrix : Matrix4x4.identity;
            var modelMatrix = modelTransform != null ? modelTransform.localToWorldMatrix : Matrix4x4.identity;
		...
		public static void SetUnityTransform(this Node node, Transform transform, bool useLocal=true)
		{
			...
			// For skinned meshes, we need to convert the transform from the root bone to the parent
			if (transform.TryGetComponent(out SkinnedMeshRenderer skinnedMeshRenderer) && skinnedMeshRenderer.rootBone != null)
			{
				var inverseTransform = MatrixBetween(transform.parent, skinnedMeshRenderer.rootBone.parent);
				position = inverseTransform.GetPosition();
				rotation = inverseTransform.rotation;
				scale = inverseTransform.lossyScale;
			}
