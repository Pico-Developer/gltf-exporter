...
public class GLTFUtils
{
    ...
    public static bool isValidMeshObject(GameObject gameObject)
    {
		if (gameObject.TryGetComponent<MeshRenderer>(out var meshRenderer))
		{
			if (gameObject.TryGetComponent<MeshFilter>(out var filter))
			{
				if (filter.sharedMesh != null && gameObject.TryGetComponent<TMP_Text>(out var tmpText))
				{
					tmpText.ForceMeshUpdate();
				}

				return filter.sharedMesh?filter.sharedMesh.vertexCount > 0 : true;
			}
